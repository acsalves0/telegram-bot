...
bot.on("message", async (msg) =>{
    ...
    if(msg.text === "/echo"){
        ...
        collection.insertOne(camarada, async (err, resultado) => {
          ...
          bot.sendMessage(chatId, `Inscrição realizada com sucesso!! ${emoji.tada} ${emoji.confetti_ball}`) 
          await Mongo.desconectaMongoDB(client);
        });
      } catch (erro) {
        ...
        bot.sendMessage(chatId, `Olá, eu sou o BotSamurai ${emoji.crossed_swords}. Estou aqui para auxiliá-lo com seu agendamento.
        As possíveis opções estão abaixo:
            /echo --> Interagir comigo
            /subscribe -> Você se inscreve para receber minhas mensagens
            /unsubscribe -> Você para de receber as mensagens
        Estou ansioso para interagir contigo    
    `);
    }
})
